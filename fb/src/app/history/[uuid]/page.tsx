...
export default function HistoryDetailPage() {
  ...
  const carouselImages = useMemo(() => {
    ...
    return [`/uploads/${data.pictureUrl}`, ...images];
  }, [data]);

  if (isLoading) return <div className="text-center p-4">Loading...</div>;
  if (!data) return <div className="text-center p-4">Data tidak ditemukan.</div>;
  ...
  return (
    <div className="flex-grow flex flex-col items-center bg-gray-100 mb-18 sm:mb-3 pt-4 text-black">
      <div className="bg-white rounded-md shadow p-4 max-w-4xl w-full">
        <h1 className="text-2xl font-bold text-center mb-4">Detail Riwayat Scan</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="sm:w-1/2 text-center">
            <h2 className="text-xl font-semibold">{data.batik?.name || 'Motif Tidak Dikenal'}</h2>
            <p className="text-gray-600">Asal: {data.batik?.originCity || '-'}</p>
